...
void EnsureClsidRegistry() {
  ...
  const wchar_t* clsid = kElectronToastActivatorClsid;
  std::wstring key_path =
      L"Software\\Classes\\CLSID\\" + std::wstring(clsid);
...
void EnsureShortcut() {
  ...
  if (FAILED(SHGetKnownFolderPath(FOLDERID_Programs, 0, nullptr, &programs_path)))
    return;
  base::FilePath programs(programs_path);
  CoTaskMemFree(programs_path);

  std::string product_name_utf8 = GetApplicationName();
  std::wstring product_name = base::UTF8ToWide(product_name_utf8);
  if (product_name.empty()) product_name = L"ElectronApp";
    ...
    if (SUCCEEDED(CoCreateInstance(CLSID_ShellLink, nullptr, CLSCTX_INPROC_SERVER,
      ...
      if (SUCCEEDED(existing.As(&pf)) &&
        ...
        if (SUCCEEDED(existing.As(&store))) {
          PROPVARIANT pv_id; PropVariantInit(&pv_id);
          PROPVARIANT pv_clsid; PropVariantInit(&pv_clsid);
          if (SUCCEEDED(store->GetValue(PKEY_AppUserModel_ID, &pv_id)) &&
              SUCCEEDED(store->GetValue(PKEY_AppUserModel_ToastActivatorCLSID, &pv_clsid))) {
            if (pv_id.vt == VT_LPWSTR && pv_clsid.vt == VT_CLSID && pv_id.pwszVal &&
                pv_clsid.puuid) {
              static const CLSID kActivatorClsidStruct = {0x9e0a8b8f,0x6f2b,0x4c63,{0xb5,0xf0,0x9b,0x1f,0x3f,0x8e,0x4d,0x1a}};
              if (_wcsicmp(pv_id.pwszVal, aumid) == 0 &&
                  IsEqualGUID(*pv_clsid.puuid, kActivatorClsidStruct)) {
                needs_update = false;
              }
            }
          }
  ...
  shell_link->SetWorkingDirectory(base::FilePath(exe).DirName().value().c_str());
  ...
  if (SUCCEEDED(shell_link.As(&prop_store))) {
    ...
    GUID clsid;
    if (SUCCEEDED(::CLSIDFromString(kElectronToastActivatorClsid, &clsid)) &&
        SUCCEEDED(InitPropVariantFromCLSID(clsid, &pv2))) {
      prop_store->SetValue(PKEY_AppUserModel_ToastActivatorCLSID, pv2);
...
void NotificationActivator::RegisterActivator() {
  ...
  CLSID clsid;
  if (FAILED(::CLSIDFromString(kElectronToastActivatorClsid, &clsid))) {
  ...
  HRESULT hr = ::CoRegisterClassObject(
      clsid, static_cast<IClassFactory*>(g_factory_instance), CLSCTX_LOCAL_SERVER,
...
void HandleToastActivation(const std::wstring& invoked_args,
  ...
  // Expected invoked_args format: type=<click|action|reply>&action=<index>&tag=<hash>
  ...
  auto* browser_client = static_cast<ElectronBrowserClient*>(ElectronBrowserClient::Get());
  if (!browser_client)
    return;
  ...
  for (auto* n : presenter->notifications()) {
    std::wstring tag_hash = base::NumberToWString(base::FastHash(n->notification_id()));
    if (tag_hash == tag_str) {
      target = n;
      break;
    }
